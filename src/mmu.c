...
static cpu_init(void)
{
	...
	*/
	...
	/*设置内存属性的索引值，mair_el1寄存器       arm-v8 P3335*/
	 ...
	 * TG1 TG0：配置页表粒度的大小，4KB or 16KB or 64KB
	 * T1SZ T0SZ 配置TTBR1_EL1 TTBR0_EL1页表能管辖的大小
	 * IPS 物理地址的位宽，48bit or 256TB大小的物理地址
	 * IRGN,ORGN:设置normal memory的共享属性 write back 
	*/
...
static int enable_mmu(void)
{
	...
	/*
	 * 虚拟地址高16位为1是选择ttbr1_el1，linux中的内核空间
	 * 虚拟地址高16位为0是选择ttbr0_el1，linux中的用户空间
	*/
	write_sysreg(idmap_pg_dir, ttbr0_el1);
	isb();

	/*
	 * sctlr_el1.M，打开和关闭mmu
	 * sctlr_el1.C，打开和关闭数据cache
	*/
