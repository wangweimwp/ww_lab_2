...
static inline pte_t clean_pte_bit(pte_t pte, unsigned long bit)
{
	pte_val(pte) &= ~bit;
	return pte;
}
static inline pte_t set_pte_young(pte_t pte)
{
	pte = set_pte_bit(pte, PTE_AF);
	return pte;
}
static inline pte_t set_pte_mkwrite(pte_t pte)
{
	pte = set_pte_bit(pte, PTE_WRITE);
	pte = clean_pte_bit(pte, PTE_RDONLY);
	return pte;
}
